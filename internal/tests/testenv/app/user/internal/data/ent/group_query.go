...
func (_q *GroupQuery) QueryAdmins() *UserQuery {
	query := (&UserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *GroupQuery) WithAdmins(opts ...func(*UserQuery)) *GroupQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) loadAdmins(ctx context.Context, query *UserQuery, nodes []*Group, init func(*Group), assign func(*Group, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.AdminsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.group_admins
		if fk == nil {
			return fmt.Errorf(`foreign-key "group_admins" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "group_admins" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
